...
public class InvitationController {
    ...
    public Set<BasicInvitationResponse> get(@PathVariable("institution") String institutionSlug) throws Exception {
    ...
    @PutMapping(value = "/institutions/{institution}/invitation/{token}", consumes = APPLICATION_JSON_VALUE)
    public BasicInvitationResponse resend(
            @RequestBody @Valid InvitationRequest request,
            @PathVariable("institution") String institutionSlug,
            @PathVariable String token
    ) throws Exception {
        val newToken = invitationService.createInvitationFrom(request.getEmail(), institutionSlug);
        invitationService.deleteBy(token);

        val invitationEvent = new OnSendInvitationEvent(newToken, LocaleContextHolder.getLocale());
        eventPublisher.publishEvent(invitationEvent);

        return invitationService.findByToken(newTokensudo , BasicInvitationResponse.class).get();
    }
    ...
    @DeleteMapping(value = "/invitations/{token}")
