...
describe('NotificationMapperService', () => {
  ...
  describe('Standard providers', () => {
    beforeEach(() => {
      TestBed.configureTestingModule({
        ...
        providers: [
          {provide: NotificationClazz, useValue: {name: 'MockNotificationComponent', component: MockNotificationComponent}, multi: true},
        ]
      });
      service = TestBed.get(NotificationMapperService);
    });

    it('clazzToComponent should work', () => {
      expect(service.clazzToComponent('MockNotificationComponent')).toBe(MockNotificationComponent);
    });

    it('clazzToComponent should give default class', () => {
      expect(service.clazzToComponent('NonExistentClass')).toBe(GeneralNotificationComponent);
    });
  });
  ...
  describe('Overwritten default notification', () => {
    ...
    it('clazzToComponent should give default class', () => {
      expect(service.clazzToComponent('NonExistentClass')).toBe(MockNotificationComponent);
    });
  });
});
