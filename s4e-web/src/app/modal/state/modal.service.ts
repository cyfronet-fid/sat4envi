...
import {
  ALERT_MODAL_ID,
  AlertModal
} from '../components/alert-modal/alert-modal.model';
import {map} from 'rxjs/operators';
import {
  CONFIRM_MODAL_ID,
  ConfirmModal,
  isConfirmModal
} from '../components/confirm-modal/confirm-modal.model';

@Injectable({providedIn: 'root'})
export class ModalService {
  constructor(
    private store: ModalStore,
  ...
  async confirm(title: string, content: string): Promise<boolean> {
    ...
    return this.query
      .modalClosed$(CONFIRM_MODAL_ID)
      .pipe(map(m => (isConfirmModal(m) ? m.returnValue : false)))
      .toPromise();
  }
}
