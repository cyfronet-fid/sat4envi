...
function isSearchedInstitution(institution: Institution, searchValue: string) {
  ...
  return (
    normalizedInstitutionName.indexOf(normalizedSearchValue) > -1 ||
...
export class ParentInstitutionModalComponent
  extends ModalComponent<Partial<Institution>>
  ...
  ngOnInit() {
    combineLatest(
      ...
      .subscribe(([institutions, searchValue]) => {
        this.searchedInstitutions =
          (!!searchValue &&
            searchValue.trim() !== '' &&
            institutions.filter(institution =>
              isSearchedInstitution(institution, searchValue)
            )) ||
          institutions;
      });
