...
import {
  convertToSceneWithPosition,
  SceneFactory
} from '../state/scene/scene.factory.spec';
...
describe('PointStacker', () => {
  ...
  it('should stack', () => {
    ...
    expect(output).toEqual([
      {points: scenesWithPositions, position: 37.5, selected: false}
    ]);
  });
  ...
  it('should partially stack', () => {
    ...
    expect(output).toEqual([
      {
        points: [scenesWithPositions[0], scenesWithPositions[1]],
        position: 12.5,
        selected: false
      },
      {
        points: [scenesWithPositions[2], scenesWithPositions[3]],
        position: 62.5,
        selected: false
      }
    ]);
  ...
  it('should not stack', () => {
    ...
    expect(output).toEqual([
      {
        points: [scenesWithPositions[0]],
        position: scenesWithPositions[0].position,
        selected: false
      },
      {
        points: [scenesWithPositions[1]],
        position: scenesWithPositions[1].position,
        selected: false
      },
      {
        points: [scenesWithPositions[2]],
        position: scenesWithPositions[2].position,
        selected: false
      },
      {
        points: [scenesWithPositions[3]],
        position: scenesWithPositions[3].position,
        selected: false
      }
    ]);
  });
...
describe('TimelineComponent', () => {
  ...
  it('should stackPoint', async () => {
    ...
    jest
      .spyOn((component as any).element.nativeElement, 'clientWidth', 'get')
    ...
    expect(await component.scenes$.pipe(take(2), toArray()).toPromise()).toEqual([
      ...
      scenes.map(
        scene =>
          ({points: [scene], selected: false, position: scene.position} as DataPoint)
      )
    ]);
  });
});
