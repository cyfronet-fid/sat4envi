...
describe('SentinelSearchResultService', () => {
  ...
  describe('getSentinels', () => {
    ...
    it('should call http and set loadings', async () => {
      ...
      const r = http.expectOne({
        method: 'GET',
        url: `${environment.apiPrefixV1}/config/search`
      });
  ...
  describe('search', () => {
    it('should work', async () => {
      ...
      const r = http.expectOne({
        method: 'GET',
        url: `${environment.apiPrefixV1}/search?param1=abc&limit=15&offset=0`
      });
      r.flush(searchResults);
      await of(null).pipe(delay(500)).toPromise();
      const rCount = http.expectOne({
        method: 'GET',
        url: `${environment.apiPrefixV1}/search/count?param1=abc&limit=15&offset=0`
      });
      rCount.flush(13);
      await promise;
      ...
      expect(query.getAll()).toEqual(
        searchResults.map(el => createSentinelSearchResult(el))
      );
      http.verify();
    });
    ...
    it('should set errors and show notification', async () => {
      ...
      const r = http.expectOne({
        method: 'GET',
        url: `${environment.apiPrefixV1}/search?limit=15&offset=0`
      });
  ...
  it('openModalForResult', () => {
    ...
    expect(spy).toHaveBeenCalledWith([], {
      replaceUrl: true,
      queryParamsHandling: 'merge',
      queryParams: {[SENTINEL_SEARCH_ACTIVE_ID]: result.id}
    });
  });
  ...
  it('connectQueryToActiveModal', async () => {
    ...
    const routerSpy = spyOn(routerQuery, 'selectQueryParams').and.returnValue(
      params$.asObservable()
    );
  ...
  it('clearResults', () => {
    ...
    expect(spy).toHaveBeenCalledWith([], {
      queryParamsHandling: 'merge',
      queryParams: {showSearchResults: undefined}
    });
    expect(query.getAll().length).toBe(0);
  });
  ...
  it('redirectToLoginPage', () => {
    const router: Router = TestBed.inject(Router);
    const notificationService: NotificationService = TestBed.inject(
      NotificationService
    );

    const spyRouter = spyOn(router, 'navigate');
    const spyNotification = spyOn(notificationService, 'addGeneral');
    const spyRouterQuery = spyOn(
      (service as any).routerQuery as RouterQuery,
      'getValue'
    ).and.returnValue({state: {url: '/map/abc'}});
    service.redirectToLoginPage();

    expect(spyRouter).toHaveBeenCalledWith(['/login'], {
      queryParams: {[BACK_LINK_QUERY_PARAM]: '/map/abc'}
    });
    expect(spyNotification).toHaveBeenCalledWith({
      type: 'warning',
      duration: 10000,
      content: 'Zaloguj się by uzyskać dostęp do zasobu'
    });
  });
  ...
  describe('connectQueryToForm', () => {
    ...
    beforeEach(() => {
      ...
      spyOn(query, 'selectShowSearchResults').and.returnValue(
