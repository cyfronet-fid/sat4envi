...
export class SentinelSearchService {
  ...
  toggleSentinelVisibility(sentinelId: string) {
    this.setSentinelVisibility(
      sentinelId,
      !this.query.isSentinelVisible(sentinelId)
    );
  }
  ...
  search(
    form: FormGroup,
    page: number = 0,
    refreshCount: boolean = true
  ): Observable<SentinelSearchResultResponse[]> {
    const params: HashMap<any> = Object.values(form.value).reduce(
    ...
    return fromPromise(
      ...
      switchMap(() =>
        this.http.get<SentinelSearchResultResponse[]>(
          `${environment.apiPrefixV1}/search`,
          {params}
        )
      ),
      delay(250),
      ...
      switchMap(data => {
        return refreshCount
          ? this.http
              .get<any>(`${environment.apiPrefixV1}/search/count`, {params})
              .pipe(
                tap(count =>
                  this.store.update({
                    resultTotalCount: count,
                    resultPagesCount: Math.ceil(count / SENTINEL_PAGE_SIZE)
                  })
                ),
                map(() => data)
              )
  ...
  getSentinels$(): Observable<SentinelSearchMetadata> {
    ...
    return this.http.get<SentinelSearchMetadata>(url).pipe(
  ...
  openModalForResult(resultId: number) {
    this.router.navigate([], {
      replaceUrl: true,
      queryParamsHandling: 'merge',
      queryParams: {[SENTINEL_SEARCH_ACTIVE_ID]: resultId}
    });
  }
  ...
  connectQueryToActiveModal(): Observable<any> {
    return of(null).pipe(
      ...
      pairwise(),
      filter(
        ([prevResultId, curResultId]) => curResultId != null && prevResultId == null
      ),
      ...
      .pipe(
        ...
        switchMap(([params, page]) =>
          this.query
            ...
            .pipe(take(1))
            .pipe(map(show => [show, params, page]))
        ),
  ...
  redirectToLoginPage() {
    ...
    this.router.navigate(['/login'], {
      queryParams: {[BACK_LINK_QUERY_PARAM]: this.routerQuery.getValue().state.url}
    });
  }
  ...
  private _clearSentinelSearchFromQuery(): Promise<any> {
    return this.router.navigate([], {
      queryParamsHandling: 'merge',
      queryParams: {[SENTINEL_SHOW_RESULTS_QUERY_KEY]: undefined}
    });
  }
