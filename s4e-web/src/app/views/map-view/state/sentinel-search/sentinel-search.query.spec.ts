...
} from './sentinel-search.factory.spec';
...
describe('SentinelSearchResultQuery', () => {
  ...
  beforeEach(() => {
    ...
    selectQueryParamsSpy = spyOn(routerQuery, 'selectQueryParams').and.returnValue(
      queryParams$
    );
  });
  ...
  it('getSelectedSentinels', () => {
    const spy = spyOn(routerQuery, 'getQueryParams').and.returnValue(
      'sentinel1,sentinel2'
    );
  ...
  it('selectSelectedSentinels', async () => {
    ...
    expect(await query.selectSelectedSentinels().pipe(take(1)).toPromise()).toEqual([
      'sentinel1',
      'sentinel2'
    ]);

    expect(routerQuery.selectQueryParams).toHaveBeenCalledWith(
      SENTINEL_SELECTED_QUERY_KEY
    );
  });
  ...
  it('selectVisibleSentinels', async () => {
    queryParams$.next('sentinel1,sentinel2');
    expect(await query.selectVisibleSentinels().pipe(take(1)).toPromise()).toEqual([
      'sentinel1',
      'sentinel2'
    ]);
  ...
  it('selectMetadataLoading', async () => {
    expect(await query.selectMetadataLoading().pipe(take(1)).toPromise()).toBe(
      false
    );

    store.setMetadataLoading(true);

    expect(await query.selectMetadataLoading().pipe(take(1)).toPromise()).toBe(true);
  });

  it('selectMetadataLoaded', async () => {
    expect(await query.selectMetadataLoaded().pipe(take(1)).toPromise()).toBe(false);

    store.setMetadataLoaded(true);

    expect(await query.selectMetadataLoaded().pipe(take(1)).toPromise()).toBe(true);
  });
  ...
  it('selectLoaded', async () => {
    expect(await query.selectLoaded().pipe(take(1)).toPromise()).toBe(false);

    store.setLoaded(true);

    expect(await query.selectLoaded().pipe(take(1)).toPromise()).toBe(true);
  });
  ...
  it('selectSentinels', async () => {
    ...
    expect(await query.selectSentinels().pipe(take(1)).toPromise()).toEqual(
      metadata
    );
  });

  it('isSentinelSelected', () => {
    const spy = spyOn(query, 'getSelectedSentinels').and.returnValue([
      'sentinel1',
      'sentinel2'
    ]);
  ...
  it('isSentinelVisible', () => {
    const spy = spyOn(query, 'getVisibleSentinels').and.returnValue([
      'sentinel1',
      'sentinel2'
    ]);
  ...
  it('selectIsActiveFirst', async () => {
    const results = SentinelSearchFactory.buildList(3).map(result =>
      createSentinelSearchResult(result)
    );
    store.set(results);
