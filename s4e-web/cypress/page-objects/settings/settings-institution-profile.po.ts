...
export class SettingsInstitutions extends Core {
  static readonly pageObject = {
    ...
    getInstitutionsCount: () =>
      cy.get('[data-e2e="institution-name"]').its('length').as('institutionsCount'),
    ...
    getAllParentInstitutionName: () =>
      cy.get('[data-e2e="selectParentInstitution"]'),
    ...
    getSecondaryPhoneInput: () =>
      cy.get('[data-e2e="secondaryPhone"]').find('input'),
  ...
  static selectNthInstitution(nth: number) {
    SettingsInstitutions.pageObject.getInstitutionsList().eq(nth).click();
  ...
  static selectInstitutionByName(name: string) {
    SettingsInstitutions.pageObject.getInstitutionsList().contains(name).click();
  ...
  static goToAddChildInstitutionPage() {
    SettingsInstitutions.pageObject.getAddChildBtn().click();
  ...
  static goToAddNewInstitutionPage() {
    SettingsInstitutions.pageObject.getAddNewInstitutionBtn().click();

    return SettingsInstitutions;
  }

  static goToEditInstitutionPage() {
    SettingsInstitutions.pageObject.getEditInstitutionBtn().click();
  ...
  static editForm(name: string, institution: InstitutionForm) {
    SettingsInstitutions.pageObject.getCityInput().clear().type(name);

    SettingsInstitutions.pageObject.getEmailInput().type(institution.adminsEmails);

    return SettingsInstitutions;
  }

  static removeInstitution(name: string) {
    SettingsInstitutions.pageObject
      .getInstitutionsList()
      .contains(name)
      .parent()
      .parent()
  ...
  static fillForm(institution: InstitutionForm, parentInstution = true) {
    if (parentInstution) {
      SettingsInstitutions.pageObject.getParentNameInput().click();
      SettingsInstitutions.pageObject.getParentModalContainer().should('be.visible');
      SettingsInstitutions.pageObject.getParentModalContainer().should('be.visible');
      SettingsInstitutions.pageObject
        .getAllParentInstitutionName()
        .first()
        .click({force: true});
      SettingsInstitutions.pageObject.getAssignToParentInstitutionBtn().click();
    }

    SettingsInstitutions.pageObject.getNameInput().type(institution.name);
    SettingsInstitutions.pageObject.getStreetInput().type(institution.address);
    SettingsInstitutions.pageObject.getZipCodeInput().type(institution.postalCode);
    SettingsInstitutions.pageObject.getCityInput().type(institution.city);
    SettingsInstitutions.pageObject.getPhoneInput().type(institution.phone);
    SettingsInstitutions.pageObject
      .getSecondaryPhoneInput()
      .type(institution.secondaryPhone);
    SettingsInstitutions.pageObject.getLogoInput().attachFile('emblem.png');
    SettingsInstitutions.pageObject.getLogoImage().should('be.visible');

    return SettingsInstitutions;
  }

  static submit() {
    SettingsInstitutions.pageObject.getSubmitBtn().click();
  ...
  static shouldDisplayInstitutionProfile(name: string) {
    ...
    cy.location('href').should(
      'include',
      `/settings/institution?institution=${institutionName}`
    );
  ...
  static errorsCountShouldBe(count: number) {
    SettingsInstitutions.pageObject.getErrors().should('have.length', count);
  ...
  static addChildAndEditInstitutionShould(value: string) {
    SettingsInstitutions.pageObject.getAddChildBtn().should(value);

    SettingsInstitutions.pageObject.getEditInstitutionBtn().should(value);
