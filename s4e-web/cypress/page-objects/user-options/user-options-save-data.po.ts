...
export class UserOptionsSaveData extends Core {
  static pageObject = {
    ...
    getOpenSaveMetaDataOnDiskBtn: () =>
      cy.get('[data-e2e="open-save-on-disk-metadata-modal"]'),
  ...
  static saveImageOnDisk() {
    UserOptionsSaveData.pageObject.getOptionsBtn().click();

    UserOptionsSaveData.pageObject
  ...
  static openSaveMetaDataOnDiskModal() {
    ...
    UserOptionsSaveData.pageObject.getOpenSaveMetaDataOnDiskBtn().click();

    UserOptionsSaveData.pageObject.getModal().should('be.visible');
