...
describe('Map layers', () => {
  context('Configuration layers for not logged in user', () => {
    ...
    it("configuration layers shouldn't be visible without authentication", () => {
      Login.goToMap();
      MapLayers.layersConfigurationAreNotVisible();
    });
  });
  ...
  context('Configuration layers for logged in user', () => {
    ...
    it('should display all url layers', function () {
      ...
      MapLayers.openManagementModal().fillForm(label, geoserverUrl);
      MapLayers.selectedUrlLayersCountShouldBe(29).allUrlLayersCountShouldBe(29);
    ...
    it('should display selected url layers on values in "layers=" query param', function () {
      const label = 'Test';
      const geoserverUrl = '/test-wms';
      const layers = ['main:opad_h05_12h', 'main:rgb24_micro'].join(',');
    ...
    it('should display selected layer', () => {
      MapLayers.selectNthSidebarLayer(0)
    ...
    it('should add, hide and remove layer from panel', function () {
      ...
      const layers = ['main:opad_h05_12h'];
      ...
      MapLayers.openManagementModal()
        ...
        .addNew();
      GeneralModal.closeModal();
      MapLayers.sidebarLayersCountShouldBe(this.layerCount + 1)
        .openManagementModal()
        .toggleNthInPanelDisplay(this.layerCount);
      GeneralModal.closeModal();
      MapLayers.sidebarLayersCountShouldBe(this.layerCount)
        .openManagementModal()
