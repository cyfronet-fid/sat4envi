...
describe('Manage products', () => {
  ...
  it('should displayed institution products', function () {
    MapProducts.selectProductByName('dobowy').selectProductByName('godzinny');
    UserOptionsGoToSettings.gotoUserProfile();
    SettingsNav.goToInstitutionsList();
    SettingsInstitutions.selectNthInstitution(0);
    SettingsManageProducts.goToManageProductsPage().productsCountShouldBe(2);
    SettingsNav.logOut();
    Login.loginAs(this.cat1user);
    MapProducts.productWithNameShouldNotBeVisible(
      'dobowy'
